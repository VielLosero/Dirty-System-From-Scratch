...
# Create new makers from UPDATEDIR_REPO_MAKERS_WITH_NEW_VER=/tmp/updates/repository/makers-with-new-ver
for file in $(ls -1 $UPDATEDIR_REPO_MAKERS_WITH_NEW_VER/* ) ; do
	echo ""
  echo "  Creating new maker for: $file"
  CHECK_RELEASE=1 NEW=0 bash $file
  if [ $? -eq 2 ] ; then 
    ln -s $(realpath $file) $UPDATEDIR_REPO_MAKERS/ && rm -v $file
  fi
done

# Show list of new makers created.
if [ -z "$(ls -1A $UPDATEDIR_REPO_MAKERS )"  ] ; then
	echo ""
	echo "No new makers found."
else
	echo ""
	echo "List of new makers."
	ls -1 $UPDATEDIR_REPO_MAKERS/ | sed 's%.*/%%g'
fi
